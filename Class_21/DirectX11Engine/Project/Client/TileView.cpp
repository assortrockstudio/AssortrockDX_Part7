...
void TileView::MouseCheck()
{
	...
	int Col = (int)OffsetMousePos.x / (int)TileEachSize.x;
	int Row = (int)OffsetMousePos.y / (int)TileEachSize.y;

	if (OffsetMousePos.x < 0.f || pTileMap->GetColumn() <= Col
		|| OffsetMousePos.y < 0.f || pTileMap->GetRow() <= Row)
	{
		return;
	}
}
